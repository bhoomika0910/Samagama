  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      <section className="rounded-[2rem] bg-white/80 p-6 shadow-soft backdrop-blur">
        ...
        <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by keyword, answer, or tags" className="input mt-6" />
      ...
      <section className="mt-8 grid gap-4">
        {faqs.map((faq, index) => (
          <FAQCard key={faq._id || index} faq={faq} open={openIndex === index} onToggle={() => setOpenIndex(openIndex === index ? -1 : index)} />
        ))}
      </section>
    </main>
  );
